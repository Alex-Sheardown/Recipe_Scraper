...
proportion_checker = ""
tablespoon_pattern = r"(\d+(?:_\d+)?)\s*[_\s]*(?:tbsp|TBSP|tbspoons|tblsp|tbspn|tbsp\.|tbsps|tbspns|tbspoons|tblsp|tbspn\.|table\s*spoons|table\s*spoonsful|tablespoons|tbs\.p\.oon\.s|tbs\s*o\s*o\s*n\s*s|t\s*b\.l\s*sp|t\s*b\s*spo\s*o\s*n\s*s|tb\s*spn|t\s*b\s*l\s*sp\.|tb\s*s\s*p\s\s*s|tbl\s*p|t\s*b\s*l\s*sp\.\s*|tbl\s*s\.p\s*)"
...
oz_pattern = r"(\d+(?:_\d+)?)\s*[_\s]*(ounce|oz(?:\.|s)?)"
fl_oz_pattern = r"(\d+(?:_\d+)?)\s*[_\s]*(fl(?:\.|uid)?[\s_]*oz|fluid[\s_]*ounce)"#r"(fl(?:\.|uid)?\s*oz|fluid\s*ounce)"
cup_pattern = r"\b(\d+(?:_\d+)?)\s*[_\s]*([cC]\s*(ups?)?\.?)\b"
qt_pattern = r"(?:\d+[\s_]*q(?:uarts?|t)(?:\.|s)?)"
pint_pattern = r'(\d+(?:_\d+)?)?\s*(?:pint|pt)s?\.?(?:\s|$)'
gallon_pattern = r'(\d+(?:_\d+)?)?\s*(?:gallon|gal)s?\.?(?:\s|$)'
