...
def check_process(base_pattern, text):
    

    combined_patterns = [
        #0 is reseverved for base
        #r'\d+(?:_\d+)?\s*(?:to)?\s*(\d+(?:_\d+)?)?\s*',
    ...
    if 1 == match_0:
        ...
        if match_1 :
            ...
            protion = average
        elif match_2 and match_2_num_a != 0:
