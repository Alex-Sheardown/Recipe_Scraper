...
def check_portion(base_pattern, text):
    ...
    if 1 == match_0:
        ...
        if match_1 :
            ...
            protion = average
            altered_text = re.sub(pattern[1], text, re.IGNORECASE)
        elif match_2 and match_2_num_a != nan:
            protion = match_2_num_a
            altered_text = re.search(pattern[2], text, re.IGNORECASE)
        elif match_3:
            protion = 3
            altered_text = re.sub(pattern[3], text, re.IGNORECASE)
        elif match_4:
            protion = 1
            altered_text = re.sub(pattern[4], text, re.IGNORECASE)

    return has_pattern, altered_text, float(protion)
...
def compare_and_categorize(row):
...
def overall_test_modifier(folder):
    ...
    data['Result'] = data.apply(compare_and_categorize, axis=1)
...
def test_pattern(given_pattern, pattern_number, match_type, folder):
        ...
        for input_string in testCase[type_P_or_F]:
            ...
            if given_pattern == False:
                ...
                has_measurement, altered_text, found_number = check_portion(given_pattern, input_string)
