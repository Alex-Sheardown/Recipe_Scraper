    ...
    for i in range(start, (start+length_per_thread)):
        ...
        if link_excepted:
            ...
            for inged in ingredients:
                ...
                """
                inged, clean_error =  clean_ingredient(inged)

                if clean_error:
                    write_To_File('recipe_info/Group_' +  str(thread_id) +'/links/error', link)
                count_ingredients = count_ingredients + 1
                """
                #start alright here we where it all begins the processing
                
                result = ic.raw_to_table_clean(inged)
                count_ingredients = count_ingredients + 1
                fmo, fme, text, mod_hash, name, portion = result
                ingredients_and_portions[0].append(portion)
                ingredients_and_portions[1].append(name)
                ingredients_and_portions[2].append(mod_hash)
                ingredients_and_portions[3].append(fmo)
