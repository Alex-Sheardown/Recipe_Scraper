...
if full_process_specific_test:
    ...
    text = "2 kg of demi-salted pork in the loin or blade"
    result_list = ic.raw_to_table_clean(text)
    count = 1
...
"""
So once the first round of cleaning goes we need to do several jobs 
1. we need to redo the collection proceess so it capture the main parts of the website
This means holding them in json or something

2. Now we need to add a count to the column section 
This can then be added so we can start removing edge dumb ass edge cases

3. We need to organize into groups in order to better handle this
First organize by count 
